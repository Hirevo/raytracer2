...
** Last update Wed Apr 19 22:50:22 2017 Arthur Knoepflin
*/
...
static sfVector3f	init_pos(void)
{
  sfVector3f		ret;
...
sfVector3f	get_pos_from_node(char *obj, char *name)
{
  sfVector3f	ret;
  char		*tmp;
  char		*pos;
  ...
  free(tmp);
  if ((tmp = get_pos(pos, 2)) == NULL)
    return (ret);
  ret.z = getnbr_parse(tmp);
  free(tmp);
  return (ret);
}
